...
def evaluate_reference_days(reference_mapping_file, dcl_prices_file, start_month=None, end_month=None):
    ...
    for _, row in ref_mapping.iterrows():
        ...
        # smetat rmse ako dvete ceni se dostapni
...
def main():
    ...
    results_df = evaluate_reference_days(reference_mapping_file, dcl_prices_file, start_month, end_month)

    results_file = '../src/data/processed/reference_days_dcl_evaluation.csv'
    results_df.to_csv(results_file, index=False)
    print(f"Results saved to {results_file}")

    if not results_df.empty:
        monthly_rmse = results_df.assign(month=pd.to_datetime(results_df['delivery_date']).dt.month).groupby('month')[
            'rmse'].mean()
